// Single-operation MX29LV320E x8 sequencer, clocked at 2.08 MHz.
...
module mx29_programmer #(
    ...
    mx29_bus bus(.clk(clk), .reset(reset), .start(bus_start),
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if ((state == POLL || state == WAIT_POLL) && timer != 24'hffffff)
                timer <= timer + 1'b1;
            case (state)
