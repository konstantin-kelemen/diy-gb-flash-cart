...
module cart_mapper (
    ...
    // gb_wr_n is asynchronous to the FPGA clock.  Do not use its edge as a
    // clock for mapper registers: at the rising edge the Game Boy may already
    // be releasing/changing the data bus.  Synchronize /WR and sample A/D only
    // after /WR has been observed LOW inside the 53.2 MHz clock domain.
    reg wr_meta=1'b1;
    reg wr_sync=1'b1;
    reg wr_seen=1'b0;

    // One clk-wide pulse, generated once per external write cycle.  At this
    // point /WR has been LOW for at least two clk samples, so gb_a/gb_data are
    // sampled well inside the write pulse instead of on its trailing edge.
    wire write_fire = !wr_sync && !wr_seen;
    ...
    always @(posedge clk or negedge gb_res_n) begin
        if(!gb_res_n) begin
            ...
            if(wr_sync) begin
                wr_seen<=1'b0;
            end else if(!wr_seen) begin
                wr_seen<=1'b1;

                if(supported && gb_rd_n && !gb_a[15]) begin
                    if(kind==MBC2) begin
                        if(!gb_a[14]) begin
    ...
    // RTC state is intentionally not reset by Game Boy RESET.  These event
    // toggles retain their FPGA power-up values and are updated from the same
    // synchronized write event as the mapper registers.
    always @(posedge clk) begin
        if(gb_res_n && write_fire && supported && gb_rd_n && has_rtc) begin
