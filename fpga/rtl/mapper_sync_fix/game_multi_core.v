...
module game_multi_core #(parameter integer RTC_CLOCK_HZ=53200000) (
    ...
    cart_mapper mapper(.clk(clk),
        .configured(configured),.multicart(multicart),.cart_type(cart_type),
    ...
    // Address decode without /CS: place the FRAM address on the shared bus
    // before the external RAM access is enabled.
    ...
    assign flash_a=!configured ? scan_address : ram_addr_window ? {5'b0,ram_address} : rom_address;
    ...
    // Приём включён между обращениями для hold регистров.
