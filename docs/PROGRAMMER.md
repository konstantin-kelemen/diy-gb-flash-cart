...
(4 194 304 байта), режим x8. FPGA — `game_programmer`, RP2040 — `programmer-v5`.
USB и SPI — v5. Game Boy отключён. [Подключение и питание](SPI_BRINGUP.md).
[Текущее состояние проверок](STATUS.md), [этапы и критерии](ROADMAP.md).

## Подготовка FPGA в Windows

Для v5 требуется [сборка FPGA из текущих исходников](../fpga/targets/game_programmer/README.md).
MAP/PAR/TRACE v5 и выпуск совместимого JEDEC пока не выполнены.
Сохранённый `releases/fpga/game_programmer/` содержит SPI v4 и с новой утилитой
и UF2 несовместим. Следующие аппаратные шаги предназначены для пары v5
после успешной сборки FPGA; аппаратное испытание v5 ещё не выполнено.
...
```sh
cp releases/rp2040/programmer-v5/gb_cart_programmer.uf2 /Volumes/RPI-RP2/
```

Плата перезапустится, диск исчезнет, появится USB CDC-порт.
[Выпуск UF2](../releases/rp2040/programmer-v5/).
...
USB-блок — до 16 КиБ, SPI-порция — до 256 байт, данные обрабатываются в FPGA.
[Описание протокола и переключения режимов](PROGRAMMER_PROTOCOL.md).
Цель чтения 4 МиБ — до 20 с; аппаратное время v5 ещё не измерено.
Теоретическая передача полезных данных по SPI занимает 8,39 с, остальные
затраты увеличивают время. После смены протокола старую утилиту не использовать.
...
Строка `Time:` отдельно показывает время стирания, проверки стирания,
программирования и сравнения ROM. Сохранить эти измерения вместе с размером
и SHA-256 ROM. Дождаться `Result: SUCCESS`. При ошибке остановить испытание.
